...
def test_img2poem(args, model, data_loader, tokenizer, device, config):
    ...
    with open(os.path.join(args.output_dir, f'results.json'),'w') as f:
        ...
        '''
        selected_text_feats, selected_text_embeds, selected_text_atts = [], [], []
        for i in tqdm(range(0, num_text, text_forward_size)):
            texts_forward = texts[i: min(num_text, i+text_forward_size)]
            ...
            text_atts = torch.cat(text_atts,dim=0)     #F, D
            sims_matrix = image_embed @ text_embeds.t() #F

            topk_sim, topk_idx = torch.topk(sims_matrix, k=config['test_k']) 
            print(text_atts.shape)
            print(topk_idx.shape, topk_sim.shape)
            print(text_atts[topk_idx,:].shape)
            selected_text_feats.append(text_feats[topk_idx,:])
            selected_text_embeds.append(text_embeds[topk_idx,:])
            selected_text_atts.append(text_atts[topk_idx,:])
        selected_text_embeds = torch.cat(selected_text_embeds,dim=0)
        selected_text_feats = torch.cat(selected_text_feats,dim=0)
        selected_text_atts = torch.cat(selected_text_atts,dim=0)
        print(selected_text_atts.shape) #170, 50, 30
        '''
    '''
    texts = data_loader.dataset.text   
    num_text = len(texts)
